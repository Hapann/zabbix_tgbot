...
async def take_incident(callback: CallbackQuery):
    ...
    async with async_session() as session:
        ...
        if incident.status == IncidentStatus.open:
            ...
            incident.assigned_to = f"@{user.username}" if user.username else user.full_name
...
async def process_comment(message: Message, state: FSMContext):
    ...
    async with async_session() as session:
        incident = await session.get(Incident, event_id)
        incident.status = IncidentStatus.closed if action == 'close' else IncidentStatus.rejected
